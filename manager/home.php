...
<div class="flex h-[calc(100vh-1rem)] md:h-[calc(100vh-2rem)] overflow-hidden gap-4">
    ...
    <main class="flex-1 flex flex-col overflow-hidden min-w-0">
        <div class="flex items-center gap-3 mb-2">
            <button onclick="toggleSidebar()" class="lg:hidden p-2 bg-white rounded-xl shadow border hover:bg-slate-50">
        ...
        <div class="flex-1 overflow-y-auto bg-white/40 rounded-[30px] p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div class="bg-white p-5 rounded-[24px] shadow-sm flex items-center justify-between border border-slate-100">
                    <div>
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-500"><i class="bi bi-collection text-xl"></i></div>
                </div>
                <div class="bg-white p-5 rounded-[24px] shadow-sm flex items-center justify-between border border-slate-100">
                    <div>
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-orange-50 flex items-center justify-center text-orange-500"><i class="bi bi-people text-xl"></i></div>
                </div>
                <div class="bg-white p-5 rounded-[24px] shadow-sm flex items-center justify-between border border-slate-100">
                    <div>
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-500"><i class="bi bi-building-check text-xl"></i></div>
                </div>
                <div class="bg-white p-5 rounded-[24px] shadow-sm flex items-center justify-between border border-slate-100">
                    <div>
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-500"><i class="bi bi-calendar-check text-xl"></i></div>
                </div>
            </div>

            <div class="mb-6">
                <div class="bg-white rounded-[28px] p-6 shadow-sm border border-slate-100">
                    <h3 class="text-sm font-bold text-slate-500 uppercase mb-4 tracking-tight">Phân bổ thành viên CLB</h3>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-white rounded-[28px] p-6 shadow-sm border border-slate-100 flex flex-col">
                    <h3 class="text-sm font-bold text-slate-500 uppercase mb-4 tracking-tight">Xu hướng đặt sân (7 ngày gần nhất)</h3>
                ...
                <div class="bg-white rounded-[28px] p-6 shadow-sm border border-slate-100">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-[10px] text-slate-400 italic bg-slate-50 px-2 py-1 rounded">Real-time</span>
                    </div>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                <?php foreach ($clubDataArray as $row): ?>
                                    <tr class="hover:bg-slate-50/50 transition-colors group">
                                        <td class="py-4 font-medium text-slate-700 group-hover:text-indigo-600 transition-colors">
                                            <?= htmlspecialchars($row['club_name']) ?>
                                        </td>
                                        <td class="py-4 text-right">
                                            <span class="px-3 py-1 bg-slate-50 text-slate-600 group-hover:bg-indigo-50 group-hover:text-indigo-600 rounded-full font-bold transition-all">
                                                <?= $row['total_members'] ?>
                                            </span>
                                        </td>
                                    </tr>
