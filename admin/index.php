    ...
    <div class="flex h-[calc(100vh-1rem)] md:h-[calc(100vh-2rem)] overflow-hidden gap-4">
        ...
        <main class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <div class="flex items-center gap-3 mb-2">
                <button onclick="toggleSidebar()" class="lg:hidden p-2 bg-white rounded-xl shadow border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div
                        ...
                        <div>
                            ...
                            <span
                                class="inline-block mt-1 px-2 py-1 text-xs rounded-full bg-emerald-100 text-emerald-700">
                                Phân quyền
                            </span>
                        </div>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
                        ...
                        <form method="GET">
                            <input name="search" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"
                                placeholder="Tìm tên hoặc email" class="px-4 py-2 border rounded-xl">
                        </form>
                    </div>
                                        ...
                                        <td class="px-6 py-3 text-right">
                                            <?php if ($u['userID'] != $_SESSION['userID']): ?>
                                                <form action="update_role.php" method="POST" class="inline-flex gap-2">
                                                    ...
                                                    <select name="roleID" class="px-3 py-1.5 rounded-lg border border-slate-200
                                                bg-white shadow-sm focus:ring-2 focus:ring-indigo-500">
                                                        <?php foreach ($roles as $r): ?>

                                                            <?php if ($r['roleID'] == 1 && $u['roleID'] != 1)
                                                                continue; ?>

                                                            <option value="<?= $r['roleID'] ?>" <?= $u['roleID'] == $r['roleID'] ? 'selected' : '' ?>             <?= $r['roleID'] == 1 ? 'disabled' : '' ?>>
                                                                <?= $r['role_name'] ?>
                                                            </option>
